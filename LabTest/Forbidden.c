...
void findpath (struct Node * p, int count, int start, int end, int * list, int *clist, int lcount,int forbidden)
{
    int index = 0, i=0;
    ...
    for (i=0;i<p[index].adjcount;i++)
    {
        ...
        if (a == 0 && p[index].adjs[i] != forbidden)
        {
            list[lcount] = p[index].adjs[i];
            clist[lcount] = p[index].costs[i];
            lcount++;
            findpath (p, count, p[index].adjs[i], end, list, clist, lcount,forbidden);
            lcount--;
        }
    }
}
int main() 
{
    ...
    int forbidden;
    scanf("%d",&forbidden);
    findpath (nodes, nodecount, start, end, list, clist, lcount,forbidden);
